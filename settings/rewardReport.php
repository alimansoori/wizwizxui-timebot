    ...
    while ($payInfo = $info->fetch_assoc()) {
        ...
        if ($payType == "INCREASE_WALLET") {
            ...
            if ($cat_id > 0) {
                ...
                if ($total_acount <= $accountCount or $total_ucount <= $accountCount) {
                ...
                for ($i = 1; $i <= $accountCount; $i++) {
                    ...
                    foreach ($files_detail as $file_detail) {
                        ...
                        if ($inbound_id == 0) {
                            if ($serverType == "marzban") {
                                $response = addMarzbanUser($server_id, $remark, $volume, $days, $plan_id);
                                if (!$response->success) {
                                ...
                                $response = addUser($server_id, $uniqid, $protocol, $port, $expire_microdate, $remark, $volume, $netType, 'none', $rahgozar, $plan_id);
                                if (!$response->success) {
                            ...
                            $response = addInboundAccount($server_id, $uniqid, $inbound_id, $expire_microdate, $remark, $volume, $limitip, null, $plan_id);
                            if (!$response->success) {
                                if (strstr($response->msg, "Duplicate email"))
                                    $remark .= RandomString();
                    ...
                    VALUES (?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, 0, ?, ?);");
                        ...
                        if ($inbound_id == 0) {
                            ...
                            $stmt->bind_param("ii", $decrement, $plan_id);
                            $stmt->execute();
                            $stmt->close();
                        }
...
\n🌐 Subscription : <code>$subLink</code>";
                        ...
                        $stmt = $connection->prepare("UPDATE `users` SET `wallet` = `wallet` + ? WHERE `userid` = ?");
                        $stmt->bind_param("ii", $eachPrice, $user_id);
                        $stmt->execute();
                        $stmt->close();
        ...
        🌐 subscription : <code>$subLink</code>
                ...
                ";
                    ...
                    VALUES (?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?,1, ?, 0, ?, ?);");
                    $stmt->bind_param("ssiiisssisiiii", $user_id, $token, $plan_id, $server_id, $inbound_id, $remark, $uniqid, $protocol, $expire_date, $vray_link, $eachPrice, $date, $rahgozar, $agentBought);
                    $stmt->execute();
                    $order = $stmt->get_result();
                    $stmt->close();
                }
                ...
                if ($inbound_id == 0) {
                    ...
                    $stmt = $connection->prepare("UPDATE `server_plans` SET `acount` = `acount` - ? WHERE id=?");
                    $stmt->bind_param("ii", $accountCount, $plan_id);
                    $stmt->execute();
                    $stmt->close();
                }
            }
