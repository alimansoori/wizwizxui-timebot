...
if (($joniedState == "kicked" || $joniedState == "left") && $from_id != $admin) {
    sendMessage(str_replace("CHANNEL-ID", $channelLock, $mainValues['join_channel_message']), json_encode([
        'inline_keyboard' => [
...
if (preg_match('/^\/([Ss]tart)/', $text) or $text == $buttonValues['back_to_main'] or $data == 'mainMenu') {
    ...
    if (isset($data) and $data == "mainMenu") {
        $res = editText($message_id, $mainValues['start_message'], getMainKeys(), "MarkdownV2");
        if (!$res->ok) {
            sendMessage($mainValues['start_message'], getMainKeys(), "MarkdownV2");
        }
    } else {
        if ($from_id != $admin && empty($userInfo['first_start'])) {
            ...
            if ($firstTimeArrivalGift > 0) {
                sendMessage("🎁 به محض ورودت، مبلغ **{$firstTimeArrivalGift} تومان** به کیف پولت اضافه شد 💳✨  

حالا می‌تونی خیلی راحت اولین سرویس‌ت رو فعال کنی:  
1️⃣ روی دکمه **«خرید سرویس جدید»** بزن\.  
2️⃣ پلن **اقتصادی** رو انتخاب کن\.  
3️⃣ در مرحله پرداخت، گزینه **پرداخت با کیف پول** رو بزن\.  

✅ همین\! سرویس‌ت بدون پرداخت اضافه فعال میشه 🚀🌍
", getMainKeys(), "MarkdownV2");
...
if (($data == "botSettings" or preg_match("/^changeBot(\w+)/", $data, $match)) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if ($data != "botSettings") {
        if ($match[1] == "cartToCartAutoAcceptType")
            $newValue = $botState[$match[1]] == "0" ? "1" : ($botState[$match[1]] == "1" ? "2" : 0);
        else
...
if (($data == "agentsList" || preg_match('/^nextAgentList(\d+)/', $data, $match)) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($keys != null)
        editText($message_id, $mainValues['agents_list'], $keys);
    else
        alert("نماینده ای یافت نشد");
}
...
if (preg_match('/^removeAgent(\d+)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($keys != null)
        ...
        editKeys(json_encode(['inline_keyboard' => [[['text' => $buttonValues['back_button'], 'callback_data' => "managePanel"]]]]));
}
...
if (preg_match('/^removePercentOfAgent(?<type>Server|Plan)(?<agentId>\d+)_(?<serverId>\d+)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($match['type'] == "Server")
        unset($discounts['servers'][$match['serverId']]);
    elseif ($match['type'] == "Plan")
        unset($discounts['plans'][$match['serverId']]);
...
if (preg_match('/^editAgentDiscount(Server|Plan|Normal)(\d+)_(.*)/', $userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    if (is_numeric($text)) {
        ...
        if ($match[1] == "Server")
            $discountInfo['servers'][$match[3]] = $text;
        elseif ($match[1] == "Plan")
            $discountInfo['plans'][$match[3]] = $text;
        elseif ($match[1] == "Normal")
            $discountInfo['normal'] = $text;
...
if (preg_match('/^edit(RewaredTime|cartToCartAutoAcceptTime)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    delMessage();
    if ($match[1] == "RewaredTime")
        $txt = "🙃 | لطفا زمان تأخیر در ارسال گزارش رو به ساعت وارد کن\n\nنکته: هر n ساعت گزارش به ربات ارسال میشه! ";
    else
        $txt = "لطفا زمان مورد نظر را به دقیقه وارد کنید";
...
if ($userInfo['step'] == "userReports" && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if (is_numeric($text)) {
        ...
        if ($keys != null) {
            ...
            sendMessage("کاربری با این آیدی یافت نشد");
    } else {
...
if ($userInfo['step'] == "transferMyWallet" && $text != $buttonValues['cancel']) {
    if (is_numeric($text)) {
        if ($text != $from_id) {
            ...
            sendMessage("میخای به خودت انتقال بدی ؟؟");
    } else
        sendMessage($mainValues['send_only_number']);
}
    ...
    if (is_numeric($text)) {
        if ($text > 0) {
            if ($userInfo['wallet'] >= $text) {
                ...
                sendMessage("موجودی حساب شما کم است");
        } else
            sendMessage("لطفا عددی بزرگتر از صفر وارد کنید");
    } else
        sendMessage($mainValues['send_only_number']);
}
...
if (preg_match('/createAccServer(\d+)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($respd->num_rows == 0) {
        ...
        while ($file = $respd->fetch_assoc()) {
            ...
            $stmt->close();
            if ($rowcount > 0)
                $keyboard[] = ['text' => "$name", 'callback_data' => "createAccCategory{$id}_{$sid}"];
        }
        if (empty($keyboard)) {
            alert("هیچ دسته بندی برای این سرور وجود ندارد");
            exit;
...
if (preg_match('/^payWithTronWallet(.*)/', $userInfo['step'], $match) && $text != $buttonValues['cancel']) {
    if (!preg_match('/^[0-9a-f]{64}$/i', $text)) {
        ...
        if ($checkExist->num_rows == 0) {
            ...
            sendMessage($mainValues['used_tax_id']);
    }
...
🌐 subscription : <code>$subLink</code>
    ...
    ", $keys, "html", $admin);
        ...
        alert("این لینک پرداخت منقضی شده است");
}
...
if ($userInfo['step'] == 's2a' and $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    sendMessage("برای همه بفرستم؟", json_encode([
        'inline_keyboard' => [
...
if ($userInfo['step'] == "forwardToAll" && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    ...
    setUser();
    sendMessage('⏳ مرسی از پیامت  ...  ', $removeKeyboard);
    sendMessage("برای همه فروارد کنم؟", json_encode([
        'inline_keyboard' => [
            [['text' => "بفرست", 'callback_data' => "yesSend2All" . $id], ['text' => "نه نفرست", 'callback_data' => "noDontSend2all" . $id]]
        ]
    ]));
...
if (preg_match('/selectCustomePlan(?<planId>\d+)_(?<categoryId>\d+)_(?<buyType>\w+)/', $data, $match) && ($botState['sellState'] == "on" || $from_id == $admin)) {
    ...
    if ($match['buyType'] == "one" && $userInfo['is_agent'] == true) {
        ...
        $discounts = json_decode($userInfo['discount_percent'], true);
        if ($botState['agencyPlanDiscount'] == "on")
            $discount = $discounts['plans'][$match[1]] ?? $discounts['normal'];
        else
...
if ($data == "getTestAccount") {
    ...
    if ($respd->num_rows > 0) {
        ...
        alert("این بخش موقتا غیر فعال است");
}

if (
    (preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/', $userInfo['step'], $match) ||
    ...
    if ($price == 0 or ($from_id == $admin)) {
        ...
        if (!preg_match('/^discountSelectPlan/', $userInfo['step']))
            $keyboard[] = [['text' => " 🎁 نکنه کد تخفیف داری؟ ", 'callback_data' => "haveDiscountSelectPlan_" . $match[1] . "_" . $match[2] . "_" . $rowId]];
...
🌐 subscription : <code>$subLink</code>";
    ...
    $msg = str_replace(
        ...
        ['کیف پول', $from_id, $username, $first_name, $price, $remark, $volume, $days],
        $mainValues['buy_custom_account_request']
    );
    sendMessage($msg, $keys, "html", $admin);
}
...
if (preg_match('/payCustomWithCartToCart(.*)/', $userInfo['step'], $match) and $text != $buttonValues['cancel']) {
    if (isset($update->message->photo)) {
        ...
        $msg = str_replace(
            ['TYPE', 'USER-ID', 'USERNAME', 'NAME', 'PRICE', 'REMARK', 'VOLUME', 'DAYS'],
            ["کارت به کارت", $from_id, $username, $first_name, $fileprice, $remark, $volume, $days],
            $mainValues['buy_custom_account_request']
        );
        $keyboard = json_encode([
            'inline_keyboard' => [
...
\n🌐 subscription : <code>$subLink</code>";
    ...
    if ($admin != $from_id) {
        ...
        $msg = str_replace(
            ...
            [$uid, $user_name, $uname, $price, $remark, $filename],
...
if (preg_match('/^servicePayWithWallet(.*)/', $data, $match)) {
    ...
    sendMessage($mainValues['please_wait_message_queue']);
    ...
    $stmt = $connection->prepare("INSERT INTO `orders_queue` (`user_id`, `token`, `cat_id`, `command`, `hash_id`, `message_id`) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isisss", $user_id, $token, $cat_id, $command, $hash_id, $message_id);
    $stmt->execute();
    $stmt->close();
...
if ($data == "requestAgency") {
    if ($userInfo['is_agent'] == 2) {
        ...
        alert($mainValues['agency_request_declined']);
    elseif ($userInfo['is_agent'] == 1)
...
if ($data == "usersOpenTickets" || $data == "userAllTickets") {
    ...
    if ($allList > 0) {
        ...
        if ($allList > $cont) {
            sendmessage("موارد بیشتر", json_encode([
                'inline_keyboard' => [
...
if (preg_match('/^latestMsg_(.*)/', $data, $match)) {
    ...
    while ($row = $chatList->fetch_assoc()) {
        ...
        $text = $row['text'];
        if (isset(json_decode($text, true)['file_id']))
            $text = "تصویر /dlPic" . $rowId;
...
if (preg_match('/^\/dlPic(\d+)/', $text, $match)) {
    ...
    while ($row = $chatList->fetch_assoc()) {
        ...
        if ($userid == $from_id || $from_id == $admin || $userInfo['isAdmin'] == true)
            sendPhoto($fileid, $caption);
    }
}
...
if (preg_match('/freeTrial(\d+)_(?<buyType>\w+)/', $data, $match)) {
    ...
    if ($match['buyType'] == "one" || $match['buyType'] == "much") {
        ...
        if ($botState['agencyPlanDiscount'] == "on")
            ...
            $discount = $discounts['servers'][$server_id] ?? $discounts['normal'];
        $price -= floor($price * $discount / 100);
    }
...
if (preg_match('/^showMainButtonAns(\d+)/', $data, $match)) {
    ...
    editText($message_id, $info['value'], json_encode([
        'inline_keyboard' => [
            [['text' => $buttonValues['back_button'], 'callback_data' => "mainMenu"]]
        ]
    ]));
...
if (preg_match('/^selectHost(?<planId>\d+)\*_\*(?<protocol>.+)\*_\*(?<tag>.*)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    $saveBtn = "ذخیره ✅";
    unset($markup[count($markup) - 1]);
    if ($markup[count($markup) - 1][0]['text'] == $saveBtn)
        unset($markup[count($markup) - 1]);
    foreach ($markup as $key => $keyboard) {
        if ($keyboard[0]['callback_data'] == $data)
            $markup[$key][0]['text'] = $keyboard['0']['text'] == $match['tag'] . " ✅" ? $match['tag'] : $match['tag'] . " ✅";
    }
...
if (preg_match('/^releaseRejectedAgent(\d+)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($keys != null) {
        ...
        editText($message_id, "کاربری یافت نشد", json_encode(['inline_keyboard' => [[['text' => $buttonValues['back_to_main'], 'callback_data' => "managePanel"]]]]));
}
...
if (preg_match('/sConfigRenew(\d+)/', $data, $match)) {
    if ($botState['sellState'] == "off" && $from_id != $admin) {
        alert($mainValues['bot_is_updating']);
        exit();
    }
...
یکی از پلن هارو انتخاب کن و برو برای پرداختش 🤲 🕋", json_encode(['inline_keyboard' => $keyboard]));
        ...
        sendMessage("💡پلنی در این دسته بندی وجود ندارد ");
}
...
if (preg_match('/(addNewRahgozarPlan|addNewPlan|addNewMarzbanPlan)/', $userInfo['step']) and $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($step == 3 and $text != $buttonValues['cancel']) {
        ...
        if (count($marzbanList) > 0)
            $condition = " AND `id` " . ($userInfo['step'] == "addNewMarzbanPlan" ? "IN" : "NOT IN") . " (" . implode(", ", $marzbanList) . ")";
        else
            $condition = "";
    ...
    if ($step == 50 and $text != $buttonValues['cancel'] and preg_match('/selectNewPlanServer(\d+)/', $data, $match)) {
        ...
        if ($userInfo['step'] != "addNewMarzbanPlan")
            ...
            editText($message_id, "📅 | لطفا تعداد روز های اعتبار این پلن را وارد کنید:");
    }
    if ($step == 51 and $text != $buttonValues['cancel'] and preg_match('/^with(Specific|Shared)Port/', $data, $match)) {
        if ($userInfo['step'] == "addNewRahgozarPlan")
            $msg = "📡 | لطفا پروتکل پلن مورد نظر را وارد کنید (vless | vmess)";
        else
            $msg = "📡 | لطفا پروتکل پلن مورد نظر را وارد کنید (vless | vmess | trojan)";
...
if (preg_match('/^wizwizplanname(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($keys == null) {
        ...
        sendMessage("ویرایش تنظیمات پلن", $keys);
}
...
if (preg_match('/^wizwizplanslimit(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($keys == null) {
        ...
        sendMessage("ویرایش تنظیمات پلن", $keys, "HTML");
}
...
if (($userInfo['step'] == "searchAgentConfig" || $userInfo['step'] == "searchMyConfig") && $text != $buttonValues['cancel']) {
    sendMessage($mainValues['please_wait_message'], $removeKeyboard);
    if ($userInfo['step'] == "searchMyConfig")
        $condition = "AND `agent_bought` = 0";
    ...
    if ($keys == null)
        sendMessage($mainValues['no_order_found']);
    else {
...
if (preg_match('/^giftServerDay(\d+)/', $userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    if (is_numeric($text)) {
        if ($text >= 0) {
            ...
            sendMessage("عددی بزرگتر و یا مساوی به 0 واردکنید");
    } else
        sendMessage($mainValues['send_only_number']);
}
...
if (preg_match('/increaseADay(.*)/', $data, $match)) {
    ...
    while ($cat = $res->fetch_assoc()) {
        ...
        if ($agentBought == true) {
            $discounts = json_decode($userInfo['discount_percent'], true);
            if ($botState['agencyPlanDiscount'] == "on")
                $discount = $discounts['plans'][$orderInfo['fileid']] ?? $discounts['normal'];
            else
                $discount = $discounts['servers'][$orderInfo['server_id']] ?? $discounts['normal'];
            $price -= floor($price * $discount / 100);
        }
        if ($price == 0)
            $price = "رایگان";
...
if (preg_match('/increaseVolumePlan(?<orderId>.+)_(?<volumeId>.+)/', $data, $match)) {
    ...
    if ($planprice == 0)
        ...
        $planprice = " " . number_format($planprice) . " تومان";
...
if (($data == 'categoriesSetting' || preg_match('/^nextCategoryPage(\d+)/', $data, $match)) and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if (isset($match[1]))
        $keys = getCategoriesKeys($match[1]);
    else
        $keys = getCategoriesKeys();
...
if (($data == 'serversSetting' || preg_match('/^nextServerPage(\d+)/', $data, $match)) and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if (isset($match[1]))
        $keys = getServerListKeys($match[1]);
    else
...
if (preg_match('/^editServerPanePassword(.*)/', $userInfo['step'], $match) and $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if (!$loginResponse['success'])
        sendMessage("اطلاعاتی که وارد کردی اشتباهه 😂");
    else {
...
if (preg_match('/^wizwizdeleteserver(\d+)/', $data, $match) and ($from_id == $admin || ($userInfo['isAdmin'] == true && $permissions['servers']))) {
    editText($message_id, "از حذف سرور مطمئنی؟", json_encode([
        'inline_keyboard' => [
...
if (preg_match('/^yesDeleteServer(\d+)/', $data, $match) && ($from_id == $admin || ($userInfo['isAdmin'] == true && $permissions['servers']))) {
    ...
    $keys = getServerListKeys();
    if ($keys == null)
        editText($message_id, "موردی یافت نشد");
    else
        editText($message_id, "☑️ مدیریت سرور ها:", $keys);
}
...
if (preg_match('/^editsServer(\D+)(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if ($text == "/empty") {
        if ($match[1] == "sni")
            $stmt = $connection->prepare("UPDATE `server_config` SET `sni` = '' WHERE `id`=?");
        elseif ($match[1] == "header_type")
            $stmt = $connection->prepare("UPDATE `server_config` SET `header_type` = 'none' WHERE `id`=?");
        elseif ($match[1] == "request_header")
            $stmt = $connection->prepare("UPDATE `server_config` SET `request_header` = '' WHERE `id`=?");
        elseif ($match[1] == "response_header")
            $stmt = $connection->prepare("UPDATE `server_config` SET `response_header` = '' WHERE `id`=?");
        elseif ($match[1] == "security")
            $stmt = $connection->prepare("UPDATE `server_config` SET `security` = 'none' WHERE `id`=?");
        elseif ($match[1] == "tlsSettings")
            $stmt = $connection->prepare("UPDATE `server_config` SET `tlsSettings` = '' WHERE `id`=?");
        ...
        elseif ($match[1] == "header_type") {
            if ($text != "http" && $text != "none") {
                ...
                $stmt = $connection->prepare("UPDATE `server_config` SET `header_type`=? WHERE `id`=?");
        } elseif ($match[1] == "request_header")
            $stmt = $connection->prepare("UPDATE `server_config` SET `request_header`=? WHERE `id`=?");
        elseif ($match[1] == "response_header")
            $stmt = $connection->prepare("UPDATE `server_config` SET `response_header`=? WHERE `id`=?");
        elseif ($match[1] == "security") {
            if ($text != "tls" && $text != "none" && $text != "xtls") {
                ...
                $stmt = $connection->prepare("UPDATE `server_config` SET `security`=? WHERE `id`=?");
        } elseif ($match[1] == "tlsSettings")
            $stmt = $connection->prepare("UPDATE `server_config` SET `tlsSettings`=? WHERE `id`=?");
        $stmt->bind_param("si", $text, $match[2]);
    }
...
if ($userInfo['step'] == "addDiscountCode" && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if (is_numeric($text)) {
        ...
        sendMessage("🔘|لطفا فقط عدد و یا درصد بفرستید");
}
...
✅ پیشنهاد ما برنامه V2Box است زیرا کار با آن ساده است و برای تمام سیستم عامل ها قابل اجرا است، میتوانید به بخش سیستم عامل مورد نظر مراجعه کنید و لینک دانلود را دریافت کنید
